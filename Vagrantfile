...
Vagrant.configure(2) do |config|
  ...
  (1..7).each do |i|
    ...
    config.vm.define "vm#{i}" do |s|
      ...
      s.vm.network "public_network", ip: "192.168.1.18#{i}", auto_config: true, bridge: "en0: Wi-Fi (AirPort)"
      s.vm.provider "virtualbox" do |v|
        v.name = "vm#{i}"
        v.memory = 2048
        v.gui = false
        unless File.exist?("vm#{i}disk2.vdi")
          v.customize ['createhd', '--filename', "vm#{i}disk2.vdi", '--format', 'VDI', '--size', 10 * 1024]
        end
